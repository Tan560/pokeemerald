...
bool8 PokemonUseItemEffects(struct Pokemon *mon, u16 item, u8 partyIndex, u8 moveIndex, bool8 usedByAI)
{
    ...
    for (i = 0; i < ITEM_EFFECT_ARG_START; i++)
    {
        ...
        case 0:
            // Cure infatuation
            if ((itemEffect[i] & ITEM0_INFATUATION) && gMain.inBattle && battler != MAX_BATTLERS_COUNT && (gBattleMons[battler].status2 & STATUS2_INFATUATION))
            {
            ...
            // Dire Hit
            if ((itemEffect[i] & ITEM0_DIRE_HIT) && !(gBattleMons[gActiveBattler].status2 & STATUS2_FOCUS_ENERGY))
            {
        ...
        case 3:
            ...
            // Cure status
            if ((itemEffect[i] & ITEM3_SLEEP) && HealStatusConditions(mon, partyIndex, STATUS1_SLEEP, battler) == 0)
            {
...
{
    if (species == SPECIES_SPINDA && dest != gMonSpritesGfxPtr->sprites.ptr[B_POSITION_PLAYER_LEFT] && dest != gMonSpritesGfxPtr->sprites.ptr[B_POSITION_PLAYER_RIGHT])
    ...
    if (species && species != SPECIES_EGG)
    {
        ...
        if (event == FRIENDSHIP_EVENT_LEAGUE_BATTLE)
        {
            ...
            if (!(gBattleTypeFlags & BATTLE_TYPE_TRAINER))
                return;
            if (!(gTrainers[gTrainerBattleOpponent_A].trainerClass == TRAINER_CLASS_LEADER || gTrainers[gTrainerBattleOpponent_A].trainerClass == TRAINER_CLASS_ELITE_FOUR || gTrainers[gTrainerBattleOpponent_A].trainerClass == TRAINER_CLASS_CHAMPION))
                return;
        }
...
bool8 TryIncrementMonLevel(struct Pokemon *mon)
{
    ...
    if (nextLevel > GetCurrentLevelCap())
    {
        return FALSE;
    }
...
void SetWildMonHeldItem(void)
{
    if (!(gBattleTypeFlags & (BATTLE_TYPE_LEGENDARY | BATTLE_TYPE_TRAINER | BATTLE_TYPE_PYRAMID | BATTLE_TYPE_PIKE)))
    {
        ...
        if (!GetMonData(&gPlayerParty[0], MON_DATA_SANITY_IS_EGG, 0) && GetMonAbility(&gPlayerParty[0]) == ABILITY_COMPOUND_EYES)
        {
            chanceNoItem = 20;
            chanceNotRare = 80;
        }
...
{
    return (species != SPECIES_CASTFORM && species != SPECIES_DEOXYS && species != SPECIES_SPINDA && species != SPECIES_UNOWN);
}
...
u16 GetRandomizedSpecies(u16 originalSpecies)
{
    ...
    // 1. Check if Randomizer Mode is ON
    // Ensure you have defined FLAG_RANDOMIZE_MODE in include/constants/flags.h
